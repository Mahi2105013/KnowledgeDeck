...
const EditNoteSection = ({ todo }) => {
  ...
  const getNote = async() => {
    try {
       ...
       let response = await fetch(`http://localhost:5000/notessec?userid=${encodeURIComponent(userid)}&wordid=${encodeURIComponent(wordid)}`);
  ...
  const updateNote = async e => {
    ...
    try {
      ...
      const response = await fetch (
        `http://localhost:5000/notessec/${todo.WORD_ID}?USER_ID=${USER_ID}`,
        {
          method: "PUT",
        ...
        <div class="modal-dialog modal-dialog-scrollable">
          <div class="modal-content" style={{backgroundColor: "white"}}>
            <div class="modal-header"> {/*the header*/}
              <h3 class="modal-title"  style={{color: 'black'}}> {todo.WORD_STRING} </h3> <br />
            ...
            <br/>
            <div class="d-flex" style={{color: 'black', textAlign: 'left'}}>
