...
const NotesList = () => {
    ...
    const getFlashcards = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/noteslist?USER_ID=${USER_ID}`);
   ...
   <div style={{
      ...
      minHeight: '100vh' // Ensures that the div is at least the height of the viewport
                ...
                <center><h1 className="mt-5">YOUR NOTES</h1></center>
                <div className="row">
                    {flashcards.map((todo) => (
                        <div key={todo.WORD_ID} className="col-sm-6 col-md-4 col-lg-3 mt-4">
                            <div className="card text-dark bg-white">
                                <div className="card-body">
                                    ...
                                    <p className="card-text"><strong>Your Note:</strong> {todo.NOTE_BODY}</p>
                                    <p> <EditNote todo = {todo} /> </p>
                                </div>
                            </div>
                        </div>
                    ))}
